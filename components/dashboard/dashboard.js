// ═══ DASHBOARD MODULE ═══
    ...
    // ═══ ENTRY-TYPE-PICKER MODULE ═══
    // Sichtbare Listbox über dem nativen <select id="inpType">. Grund: ein <option>
    // kann nur Text rendern — für SVG-Icons statt Emojis braucht es eigenes Markup.
    // Das <select> bleibt DIE Wert-Quelle (alle .value-Zugriffe im Bestandscode
    // laufen unverändert weiter), hier wird nur gespiegelt.
    (function () {
        'use strict';
        ...
        function els() {
            return {
                picker:  document.getElementById('entryTypePicker'),
                sel:     document.getElementById('inpType'),
                trigger: document.getElementById('entryTypeTrigger'),
                icon:    document.getElementById('entryTypeIcon'),
                label:   document.getElementById('entryTypeLabel'),
                panel:   document.getElementById('entryTypeList')
            };
        }

        // Typ-Farbe als "r,g,b" — dieselbe Farbcodierung wie Donut & Legende.
        function typeRgb(id) {
            try {
                if (typeof getTypeColor === 'function' && typeof hexToRgbStr === 'function') return hexToRgbStr(getTypeColor(id));
            } catch (e) { /* Typ unbekannt → Primary-Fallback aus dem CSS */ }
            return null;
        }

        function iconHTML(id) {
            try {
                if (typeof getTypeIconHTML === 'function') return getTypeIconHTML(id, 16);
            } catch (e) { /* Daten noch nicht geladen → leere Kachel statt Absturz */ }
            return '';
        }

        function buildTile(cls, typeId) {
            const tile = document.createElement('span');
            tile.className = cls;
            const rgb = typeRgb(typeId);
            if (rgb) tile.style.setProperty('--type-rgb', rgb);
            tile.innerHTML = iconHTML(typeId); // konstante SVG-Pfade bzw. esc()-tes User-Emoji
            return tile;
        }

        // Baut die Options-Liste aus den <option>s des Selects nach (inkl. Custom-Types).
        function renderEntryTypePicker() {
            const { picker, sel, panel } = els();
            if (!picker || !sel || !panel) return;
            panel.innerHTML = '';
            Array.from(sel.options).forEach((opt, i) => {
                ...
                row.append(buildTile('type-option__icon', opt.value), txt, check);
                row.addEventListener('click', () => choose(opt.value));
                row.addEventListener('mouseenter', () => highlight(i, false));
        ...
        function syncTrigger() {
            const { sel, icon, label, panel } = els();
            if (!sel || !icon || !label) return;
            const opt = sel.options[sel.selectedIndex] || sel.options[0];
            if (!opt) return;
            label.textContent = opt.textContent;
            const rgb = typeRgb(opt.value);
            if (rgb) icon.style.setProperty('--type-rgb', rgb); else icon.style.removeProperty('--type-rgb');
            icon.innerHTML = iconHTML(opt.value);
        ...
        function choose(value) {
            ...
            sel.dispatchEvent(new Event('change', { bubbles: true })); // toggleTimeInputs() + Custom-Fields
            closePicker(true);
        }

        function highlight(i, focus) {
            const { panel } = els();
            if (!panel) return;
        ...
        function openPicker() {
            const { picker, panel, sel, trigger } = els();
            ...
            highlight(Math.max(0, sel ? sel.selectedIndex : 0), true);
            document.addEventListener('mousedown', onDocDown, true);
        }

        function closePicker(refocus) {
            const { picker, trigger } = els();
            if (!picker) return;
        ...
        function onPanelKey(e) {
            ...
            if (!count) return;
            switch (e.key) {
                case 'ArrowDown': e.preventDefault(); highlight(activeIndex + 1, true); break;
                case 'ArrowUp':   e.preventDefault(); highlight(activeIndex - 1, true); break;
                case 'Home':      e.preventDefault(); highlight(0, true); break;
                case 'End':       e.preventDefault(); highlight(count - 1, true); break;
        ...
        // Bestandscode setzt inpType.value direkt (Voice-Input, Draft-Restore, NFC, Reset).
        // Ein `change` feuert dabei NICHT — deshalb hängen wir uns einmalig in den
        // value-Setter dieser einen Instanz, statt jeden Aufrufer anzufassen.
        function bindValueMirror(sel) {
            ...
            if (desc && desc.get && desc.set) {
                Object.defineProperty(sel, 'value', {
                    ...
                    set(v) { desc.set.call(this, v); syncTrigger(); }
                });
            }
            sel.addEventListener('change', syncTrigger);
        }

        function initEntryTypePicker() {
            const { picker, sel, trigger, panel } = els();
            if (!picker || !sel || !trigger || !panel || picker._inited) return;
            picker._inited = true;
            bindValueMirror(sel);
            trigger.addEventListener('click', () => { isOpen() ? closePicker(false) : openPicker(); });
            trigger.addEventListener('keydown', onTriggerKey);
            panel.addEventListener('keydown', onPanelKey);
            renderEntryTypePicker();
        }

        if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initEntryTypePicker);
        else initEntryTypePicker();

        window.renderEntryTypePicker = renderEntryTypePicker;
    })();
