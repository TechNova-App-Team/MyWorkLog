...
const SOLL_KEYS = ['betrieb_id', 'azubi_id', 'client_id', 'jahr', 'kw', 'datum_von',
    'datum_bis', 'inhalt', 'status', 'quelle', 'ki_erzeugt', 'geloescht_at'].sort();
ok(Object.keys(zeile).sort().join() === SOLL_KEYS.join(),
    'Zeile hat genau die erwarteten Spalten (ohne created_at/updated_at — Trigger)',
...
// ── berichtZuZeile — geloescht_at auf null setzen ──────────────────────
gruppe('berichtZuZeile — geloescht_at handling');
const z1 = I.berichtZuZeile({ id: 'r1', jahr: 1, kw: 1 }, 'b1', 'az1');
