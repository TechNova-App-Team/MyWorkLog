...
const DATEIEN = [
    'tools/email-templates/magic-link.html',
    'tools/email-templates/magic-link-wortmarke.html',
];
...
for (const DATEI of DATEIEN) {
    roh = fs.readFileSync(path.join(ROOT, DATEI), 'utf8').split('\r\n').join('\n');
    h = roh.replace(/<!--[\s\S]*?-->/g, '');   // ohne Kommentare
    console.log('\n═══ ' + DATEI.replace('tools/email-templates/', '') + ' ═══');
    ...
    console.log('\n4. Robustheit');
    {
        ...
        // Gross- wie Kleinschreibung, und mit Leerraum drumherum: die
    // Wortmarken-Fassung setzt MYWORKLOG mit Umbruechen im div.
    ok('Markenname existiert als Text, nicht nur im Bild',
           />\s*MyWorkLog\s*</i.test(h));
    ...
    // ── Nur die Wortmarken-Fassung ──────────────────────────────────────────
    // 🔴 Die Groesse ist gerechnet, nicht geschaetzt: kein Client garantiert
    // dieselbe Schrift. Gegen die breiteste plausible Fallback-Schrift
    // (DejaVu Bold, ~0.76em je Grossbuchstabe) muss das Wort in die 520px
    // Innenbreite passen. Wer den Text aendert, faellt hier durch.
    const marke = h.match(/class="mark"[^>]*font-size:(\d+)px[^>]*>\s*([A-Z]+)/);
    if (marke) {
        const px = Number(marke[1]), wort = marke[2];
        const breit = Math.round(wort.length * 0.76 * px - px * 0.04 * (wort.length - 1));
        ok('Wortmarke passt in die Karte (breiteste Fallback-Schrift)', breit <= 520,
           wort + ' bei ' + px + 'px → ' + breit + 'px von 520px');
        ok('Wortmarke schrumpft auf schmalen Schirmen',
           /\.mark\s*\{[^}]*font-size:\s*\d+px\s*!important/.test(h));
        ok('Wortmarke hat im Dunkelmodus eine eigene Farbe',
           /prefers-color-scheme: dark[\s\S]*?\.mark\s*\{[^}]*color:/.test(h));
        // Gegenprobe: die Regex hat wirklich etwas gefunden.
        ok('Gegenprobe — Wortmarke erkannt', px > 40 && wort.length > 4, wort + '/' + px);
    }
}

console.log(`\n${geprueft - fehler}/${geprueft} bestanden`);
if (geprueft < 60) { console.log('ZU WENIG PRUEFUNGEN — der Lauf hat nichts getan'); process.exit(1); }
